...
###   PALLY1core.org and PALLY1.org and compares them.
...
WORKINGDIR="/tmp/PALLY1_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://PALLY1core.org"
HOST2="https://PALLY1.org"
BASEDIR="/bin/"
VERSIONPREFIX="PALLY1-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'PALLY1-',
...
if [ $? -ne 0 ]; then
   echo "PALLY1.org failed to provide signature file, but PALLY1core.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "PALLY1.org and PALLY1core.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the PALLY1 Core binary release signing key installed?"
   fi
