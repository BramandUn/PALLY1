...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete PALLY1 database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no PALLY1 datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old PALLY1 datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected PALLY1 0.7 datadir."
    ;;
  3)
    echo "Detected PALLY1 pre-0.8 datadir."
    ;;
  4)
    echo "Detected PALLY1 0.8 datadir."
    ;;
esac
