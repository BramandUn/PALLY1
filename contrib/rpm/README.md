...
The RPM spec file provided here is for PALLY1-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://PALLY1.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[PALLY1 git repository](https://github.com/PALLY1/PALLY1) but are not part of
...
Currently only `Source10` is used, for the example `PALLY1.conf` file.
...
in the PALLY1 git repository. At present only an SVG version of the PALLY1
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the PALLY1
...
needed when 0.12.1 is released, a proper fix is already in the PALLY1 git
master branch.
...
idea to build PALLY1 Core against a static version of that library compiled
...
The `PALLY1.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `PALLY1-server` package. At this point, most RPM based
...
The `PALLY1-server` package should have SELinux support. How to properly do
...
PALLY1-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/PALLY1/). He has been
packaging PALLY1 for Fedora at least since 2012.
...
4. I renamed the `PALLY1` package that contains the Qt GUI to `PALLY1-core` as
that appears to be how the general population refers to it, in contrast to
`PALLY1-xt` or `PALLY1-classic`. I wanted to make sure the general population
