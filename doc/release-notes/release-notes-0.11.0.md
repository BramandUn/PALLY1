PALLY1 Core version 0.11.0 is now available from:

  <https://PALLY1.org/bin/PALLY1-core-0.11.0/>
...
For example, add the following to `PALLY1.conf`:
...
related to the blockchain in the PALLY1 system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows PALLY1 Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that PALLY1 Core will 
...
For an example script see [PALLY1-submittx](https://github.com/laanwj/PALLY1-submittx).
...
- #5334 `cf87536` libPALLY1consensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new PALLY1-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only PALLY1AmountField
- #5753 `0f44672` Add PALLY1 logo to about screen
...
- #5809 `46bfbe7` Add PALLY1-cli man page
...
- #5849 `41259ca` contrib/init/PALLY1d.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename PALLY1-Qt.app to PALLY1-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for PALLY1-tx)
