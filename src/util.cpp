...
const char * const BITCOIN_CONF_FILENAME = "PALLY1.conf";
const char * const BITCOIN_PID_FILENAME = "PALLY1d.pid";

//PALLY1 only features
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "PALLY1d -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PALLY1
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PALLY1
    // Mac: ~/Library/Application Support/PALLY1
    // Unix: ~/.PALLY1
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PALLY1Core";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PALLY1Core";
#else
    // Unix
    return pathRet / ".PALLY1core";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure PALLY1 Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("PALLY1 Core") == std::string::npos) {
